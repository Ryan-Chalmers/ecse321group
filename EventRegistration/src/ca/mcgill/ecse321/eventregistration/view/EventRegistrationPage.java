...
public class EventRegistrationPage extends JFrame {
	...
	private Integer selectedParticipant;
	...
	private void refreshData() {
		...
		if (error == null || error.length() == 0) {
			...
			Iterator<Participant> pIt = rm.getParticipant().iterator();
			Integer index = 0;
			while (pIt.hasNext()){
			...
			participantList.removeAllItems();
			Iterator<Participant> eIt = rm.getParticipant().iterator();
			index = 0;
			while (eIt.hasNext()){
				Participant e = eIt.next();
				participants.put(index, e);
				participantList.addItem(e.getName());
				index++;
			}
			selectedEvent = -1;
		...
		//pack to change the window depending on whether and error message was shown
		pack();
	}
